...
async function findYouTube(
  ...
  try {
    ...
    console.warn(`  yt-search error for ${song.artist} - ${song.title}:`, (err as Error).message);
    return null;
  }
}
...
async function main() {
  ...
  console.log(`Fetching metadata for ${seed.length} songs...`);

  const out: Song[] = [];
  const missing: SeedSong[] = [];

  for (let i = 0; i < seed.length; i++) {
    const s = seed[i];
    process.stdout.write(`[${i + 1}/${seed.length}] ${s.id} ${s.artist} - ${s.title} ... `);

    const [yt, preview] = await Promise.all([findYouTube(s), findItunesPreview(s)]);

    const duration = yt?.durationSec ?? null;
    const youtubeStartSec = duration !== null && duration < 120 ? 15 : 30; // TODO: manual refinement

    const enriched: Song = {
      ...s,
      youtubeId: yt?.id ?? null,
      youtubeStartSec,
      itunesPreviewUrl: preview,
    };
    out.push(enriched);

    const flags = [yt ? "YT" : "--", preview ? "iT" : "--"].join(" ");
    console.log(flags);

    if (!yt && !preview) missing.push(s);

    await sleep(250); // be nice to both services
  }

  await writeFile(OUT_PATH, JSON.stringify(out, null, 2) + "\n", "utf8");
  console.log(`\nWrote ${out.length} songs to ${OUT_PATH}`);

  const ytCount = out.filter((s) => s.youtubeId).length;
  const itCount = out.filter((s) => s.itunesPreviewUrl).length;
  console.log(`YouTube hits : ${ytCount}/${out.length}`);
  console.log(`iTunes hits  : ${itCount}/${out.length}`);

  if (missing.length > 0) {
    console.log(`\n${missing.length} songs missing BOTH sources — swap these out:`);
